...
const (
	...
	Carbon2FormatMetricIncludesField = "field_separate"
	Carbon2FormatFieldSeparate       = "metric_includes_field"
)
...
func TestSerializeAndParseReturnsTheSameMetric(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			serializerFormat: string(Carbon2FormatMetricIncludesField),
		},
		{
			serializerFormat: string(Carbon2FormatFieldSeparate),
		},
	}

	for _, tc := range testcases {
		t.Run(tc.serializerFormat, func(t *testing.T) {
			s := carbon2.Serializer{
				Format:              tc.serializerFormat,
